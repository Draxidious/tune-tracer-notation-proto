    ...
    */
    ...
    addNote = function (/*string[]*/keys, /*string*/ duration, /*string*/ noteId) {
        ...
        this.voice1.getTickables().forEach(function (staveNote) {
            ...
            // If invalid duration nothing changes
            if (staveNote.attrs.id == noteId && duration == staveNote.duration) {
    ...
    modifyRest = function (duration, /*string*/ noteId) {

        if (duration[duration.length - 1] != 'r') {
            // We don't add notes here
            return;
        }
        let VF = Vex.Flow;
        let notes = [];
        let originalTicks = this.voice1.getTicks();
        this.voice1.getTickables().forEach(function (staveNote) {
        ...
        // We replace the desired rest with a series of new rests that represent the new duration
        // We then count the Ticks in this new set of notes, if there are too many or too few its invalid
        // ^ AKA if the newTicks != oldTicks then invalid
